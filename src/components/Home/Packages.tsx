...
import { FolderOpenFilled, FormatPainterOutlined, CodeOutlined, } from '@ant-design/icons'
...
export default function Packages(props: Props): JSX.Element {
	...
	const PackagesElements: JSX.Element[] = [
		...
		<PackageCard key={2} imgSrc={Website} imgAlt={'Laptop with code editor open'} metaTitle={process.env.REACT_APP_SERVICE_PACKAGE_2_TITLE || ''} metaDesc={process.env.REACT_APP_SERVICE_PACKAGE_2_DESC || ''} metaOne={process.env.REACT_APP_SERVICE_PACKAGE_2_POINT_1 || ''} metaTwo={process.env.REACT_APP_SERVICE_PACKAGE_2_POINT_2 || ''} metaThree={process.env.REACT_APP_SERVICE_PACKAGE_2_POINT_3 || ''} startingPrice={process.env.REACT_APP_SERVICE_PACKAGE_2_PRICE || ''} service={2} />,
		<PackageCard key={3} imgSrc={Develop} imgAlt={'A smartpohne with apps open'} metaTitle={process.env.REACT_APP_SERVICE_PACKAGE_3_TITLE || ''} metaDesc={process.env.REACT_APP_SERVICE_PACKAGE_3_DESC || ''} metaOne={process.env.REACT_APP_SERVICE_PACKAGE_3_POINT_1 || ''} metaTwo={process.env.REACT_APP_SERVICE_PACKAGE_3_POINT_2 || ''} metaThree={process.env.REACT_APP_SERVICE_PACKAGE_3_POINT_3 || ''} startingPrice={process.env.REACT_APP_SERVICE_PACKAGE_3_PRICE || ''} service={3} />
	]

	const PackageIcons: JSX.Element[] = [<FormatPainterOutlined />, <CodeOutlined />]
